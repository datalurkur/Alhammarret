...
namespace Alhammarret
{
    ...
    public class CardDB
    {
        ...
        private HunspellWrapper hunspell;
        ...
        {
            List<string> suggestions = this.hunspell.GetSuggestions(misspelled);
            for (int i = 0; i < suggestions.Count; ++i)
            {
                if (cardByName.ContainsKey(suggestions[i]))
                {
                    return cardByName[suggestions[i]];
                }
            }
            return null;
        }
        ...
        public async void Build()
        {
            try
            {
                System.Windows.Resources.StreamResourceInfo info = Application.GetResourceStream(new Uri("Assets/Allsets.txt", UriKind.Relative));
                string jsonStr;
                using (StreamReader rdr = new StreamReader(info.Stream))
                {
                    jsonStr = await rdr.ReadToEndAsync();
                }
        ...
        private async Task BuildHunspellDictionary()
        {
            ...
            string dictPath = Path.Combine(folder, "hunspell_dict.txt");
            if (!File.Exists(dictPath))
            {
                List<string> distinct = cardByName.Values.Select(n => n.Name.ToLower()).ToList();

                using (FileStream fs = new FileStream(dictPath, FileMode.Create))
                using (StreamWriter writer = new StreamWriter(fs))
                {
                    for (int i = 0; i < distinct.Count; ++i)
                    {
                        await writer.WriteLineAsync(distinct[i]);
                    }
                }
            }

            string affixPath = Path.Combine(folder, "hunspell_affix.txt");
            if (!File.Exists(affixPath))
            {
                System.Windows.Resources.StreamResourceInfo info = Application.GetResourceStream(new Uri("Assets/hunspell_affix.txt", UriKind.Relative));
                string affixData;
                using (StreamReader rdr = new StreamReader(info.Stream))
                {
                    affixData = await rdr.ReadToEndAsync();
                }
                File.WriteAllText(affixPath, affixData);
            }

            Debug.WriteLine($"Building hunspell from {affixPath} and {dictPath}");
            this.hunspell = new HunspellWrapper(dictPath, affixPath);
        }
    }
}